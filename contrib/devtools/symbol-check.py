...
# temporary utility for identifying RISCV
# https://github.com/lief-project/LIEF/pull/562
# to be replaced with binary.header.machine_type == lief.ELF.ARCH.RISCV
def is_RISCV(binary) -> bool:
    return binary.interpreter == '/lib/ld-linux-riscv64-lp64d.so.1'
...
def check_imported_symbols(filename) -> bool:
    ...
    for symbol in binary.imported_symbols:
        ...
        machine = 'RISCV' if is_RISCV(binary) else binary.header.machine_type
        ...
        if version:
            ...
            if aux_version and not check_version(MAX_VERSIONS, aux_version, machine):
...
def check_exported_symbols(filename) -> bool:
    ...
    for symbol in binary.dynamic_symbols:
        if not symbol.exported:
            continue
        name = symbol.name
        if is_RISCV(binary) or name in IGNORE_EXPORTS:
            continue
