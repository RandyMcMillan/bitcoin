...
public:
    ...
    Balance GetBalance(int min_depth = 0, bool avoid_reuse = true) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    ScriptPubKeyMan* AddWalletDescriptor(WalletDescriptor& desc, const FlatSigningProvider& signing_provider, const std::string& label) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
};
