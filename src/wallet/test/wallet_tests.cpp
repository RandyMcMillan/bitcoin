...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    // Verify ScanForWalletTransactions fails to read an unknown start block.
    {
        ...
        BOOST_CHECK_EQUAL(WITH_LOCK(wallet.cs_wallet, return wallet.GetBalance().m_mine_immature), 0);
    }
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(WITH_LOCK(wallet.cs_wallet, return wallet.GetBalance().m_mine_immature), 100 * COIN);
    }
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(result.last_scanned_block, newTip->GetBlockHash());
        BOOST_CHECK_EQUAL(*result.last_scanned_height, newTip->nHeight);
        BOOST_CHECK_EQUAL(WITH_LOCK(wallet.cs_wallet, return wallet.GetBalance().m_mine_immature), 50 * COIN);
    }
