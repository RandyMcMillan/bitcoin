...
public:
    ...
    TransactionRecord* index(interfaces::Wallet& wallet, const uint256& last_block_hash, const int idx)
    {
        if(idx >= 0 && idx < cachedWallet.size())
        {
            ...
            int numBlocks;
            int64_t block_time;
            if (rec->statusUpdateNeeded(last_block_hash) && wallet.tryGetTxStatus(rec->hash, wtx, numBlocks, block_time)) {
                rec->updateStatus(wtx, last_block_hash, numBlocks, block_time);
            }
            return rec;
        }
        return nullptr;
...
{
    Q_UNUSED(parent);
    TransactionRecord* data = priv->index(walletModel->wallet(), walletModel->clientModel().getLastBlockHash(), row);
    if(data)
    {
