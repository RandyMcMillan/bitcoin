...
uint256 ClientModel::getLastBlockHash() const
{
    LOCK(m_cached_tip_mutex);
    if (m_last_block_hash.IsNull()) {
        m_last_block_hash = m_node.getLastBlockHash();
    }
...
static void BlockTipChanged(ClientModel* clientmodel, SynchronizationState sync_state, const uint256 blockHash, int height, int64_t blockTime, double verificationProgress, bool fHeader)
{
    if (fHeader) {
        ...
        WITH_LOCK(clientmodel->m_cached_tip_mutex, clientmodel->m_last_block_hash = blockHash;);
    }
