...
void TransactionRecord::updateStatus(const interfaces::WalletTxStatus& wtx, const uint256& block_hash, int numBlocks, int64_t block_time)
{
    ...
    status.m_last_block_hash = block_hash;
...
bool TransactionRecord::statusUpdateNeeded(const uint256& blockHash) const
{
    return status.m_last_block_hash != blockHash || status.needsUpdate;
}
