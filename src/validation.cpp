...
static void LimitMempoolSize(interfaces::TxMempool& txmempool, size_t limit, std::chrono::seconds age)
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main, txmempool.mutex())
{
    LockAssertion lock(txmempool.pool()->cs);
    LimitMempoolSize(txmempool.pool(), limit, age);
}
...
static void UpdateMempoolForReorg(interfaces::TxMempool& txmempool, DisconnectedBlockTransactions& disconnectpool, bool fAddToMempool)
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main, txmempool.mutex())
{
    if (!txmempool.pool()) return;
    CTxMemPool& mempool = *txmempool.pool();
    AssertLockHeld(cs_main);
    AssertLockHeld(txmempool.mutex());
    ...
    while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
        ...
        if (!fAddToMempool || (*it)->IsCoinBase() ||
            ...
            txmempool.RemoveRecursive(**it, MemPoolRemovalReason::REORG);
    ...
    txmempool.UpdateTransactionsFromBlock(vHashUpdate);

    // We also need to remove any now-immature transactions
    txmempool.RemoveForReorg(&::ChainstateActive().CoinsTip(), ::ChainActive().Tip()->nHeight + 1, STANDARD_LOCKTIME_VERIFY_FLAGS);
    // Re-limit mempool size, in case we added any transactions
    LimitMempoolSize(txmempool, gArgs.GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000, std::chrono::hours{gArgs.GetArg("-mempoolexpiry", DEFAULT_MEMPOOL_EXPIRY)});
}
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        CoinsCacheSizeState cache_state = GetCoinsCacheSizeState(m_txmempool->pool());
        LOCK(cs_LastBlockFile);
...
{
    AssertLockHeld(cs_main);
    AssertLockHeld(m_txmempool->mutex());
    ...
    if (disconnectpool && m_txmempool->pool()) {
        AssertLockHeld(m_txmempool->mutex());
        ...
        while (disconnectpool->DynamicMemoryUsage() > MAX_DISCONNECTED_TX_POOL_SIZE * 1000) {
            ...
            m_txmempool->RemoveRecursive(**it, MemPoolRemovalReason::REORG);
...
bool CChainState::ActivateBestChain(BlockValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
    ...
    do {
        ...
        {
            LOCK(::cs_main);
            LOCK(m_txmempool->mutex()); // Lock transaction pool for at least as long as it takes for connectTrace to be consumed
...
bool CChainState::InvalidateBlock(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex *pindex)
{
    ...
    while (true) {
        ...
        LOCK(::cs_main);
        LOCK(m_txmempool->mutex()); // Lock for as long as disconnectpool is in scope to make sure UpdateMempoolForReorg is called after DisconnectTip without unlocking in between
