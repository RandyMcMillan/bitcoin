 ...
 */
    ...
    OP_CHECKLOCKTIMEVERIFY = 0xb1,    // mark tx invalid if the stack is empty; or the top stack item < 0; or the top stack item > tx's nLockTime field or
    OP_NOP2 = OP_CHECKLOCKTIMEVERIFY, // the lock-time type (height vs. timestamp) of the top stack item != the nLockTime field; or the nSequence field of the txin = 0xffffffff;
